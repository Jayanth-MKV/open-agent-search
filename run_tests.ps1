...
switch ($choice) {
    ...
    "4" {
        ...
        locust --host=http://localhost:8000 --users 100 --spawn-rate 10 --run-time 3m --headless
    }
    "5" {
        Write-Host "Running Aggressive Rate Limit Test (AggressiveUser)..." -ForegroundColor Green
        locust --host=http://localhost:8000 --user AggressiveUser --users 30 --spawn-rate 10 --run-time 1m --headless
    }
    "6" {
        Write-Host "Running Burst Traffic Test (BurstUser)..." -ForegroundColor Green
        locust --host=http://localhost:8000 --user BurstUser --users 50 --spawn-rate 50 --run-time 30s --headless
    }
    "7" {
        ...
        locust --host=http://localhost:8000 --users $users --spawn-rate $spawnRate --run-time $runtime --headless
    }
    default {
